...
class Zend_Controller_Action_Helper_Util extends Zend_Controller_Action_Helper_Abstract
{
	...
	public function reservaSelos($quantidade){
    	
		$model_selo = new Selo();
		
    	$quantidadeSelos = $model_selo->getSelo(1);
		//$model_selo->update($data, "idPapel = " . $this->_request->getPost('idPapel'));		
    	
    	$lote = null;    	
        foreach($quantidadeSelos as $quant){
        ...
        $data['quantidade'] = $lote->quantidade - $quantidade;         
        
        $model_selo->update($data, "idSelo = " . $lote->idSelo);
