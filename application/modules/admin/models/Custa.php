...
//class Admin_Model_Custa{
	...
	public function getCustas_certidao(){
    	...
    	$select->from(array('cus' => 'car_custas'), array('nome', 'valor'));
    	
    	$select->joinInner(array('vig' => 'car_vigencias'), '', array());

    	$select->where('vig.idVigencia = (SELECT MAX(idVigencia) FROM car_vigencias)');
