...
class Begin_Atos{
	...
	public function relatorioAtoss($data_atos, $idSituacao=''){
		...
		$auxApresentante = '';
		foreach($data_atos as $titulo){
			
			$apresentante =  $titulo->nomeapresentante;
			
			if($apresentante != $auxApresentante){
				$auxApresentante = $apresentante;
				...
				$pdf->SetY(1);
				$pdf->Cell(20, 1, "REPÚBLICA FEDERATIVA DO BRASIL" ,0,1,'C');
				$pdf->SetFont('arial', 'B', 12);
				$pdf->SetY(1.8);
				$pdf->Cell(20, 1, "TABELIONATO DE PROTESTO DE TÍTULOS DE " . $this->upper($data_cartorio->cidade),0,1,'C');
				$pdf->SetY(2.3);
				...
				$pdf->SetY(3.5);
				$pdf->Cell(20, 1, 'Apresentante: ' . $auxApresentante,0,0,'L');
				$pdf->SetY(3.5);
				$pdf->Cell(18.6, 1, 'Pago em ' . date('d/m/Y'),0,0,'R');
			}
	...
	public function motivo($idSituacao){
		...
		if($idSituacao == 4){
			return 'PARA SUSTAÇÃO';
		}
		if($idSituacao == 9){
			return 'PARA SUSPENSÃO';
		}
		if($idSituacao == 24){
			return 'PARA DEVOLUÇÃO';
		}
