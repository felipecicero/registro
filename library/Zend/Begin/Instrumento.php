...
class Begin_Instrumento extends FPDF{
	...
	public function gerarInstrumento($data_devedor, $data_cartorio, $data_custas, $data_autoridade){
		...
		$pdf->Cell(7, 0.5, $this->upper($data_devedor->especie), 1, 0, 'C');
		...
		$pdf->SetXY(6, 8.8);
		$pdf->SetFont('arial', 'B', 9);
		$pdf->Cell(7, 0.5, $this->upper($data_devedor->especie), 0, 0, 'L');
		...
		$pdf->SetXY(16.5, 14.8);
		$pdf->SetFont('arial', '', 7);
		$pdf->Cell(3.6, 0.1, "Selo", 0, 0, 'C');
		$pdf->SetXY(16.5, 15.1);
		$pdf->SetFont('arial', 'B', 7);
		$pdf->Cell(3.6, 0.1, $data_devedor->selo, 0, 0, 'C');
		...
		for($i=0; $i<count($data_custas); $i++){						
			$pdf->SetXY($x, $y);		
			$pdf->Cell(1, 0.5, $this->upper($data_custas[$i]->nome) . ": " . $this->converte($data_custas[$i]->valor), 0, 0, 'R');
		...
		$pdf->Cell(3, 0.5, "ESPÉCIE: " . $this->upper($data_devedor->especie), 0, 0, 'L');		
